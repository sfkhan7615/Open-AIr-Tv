...
//  Import All Route Files from Route Dir

// Add,update,delete,get Videos
const videoRoute = require("./ApiRoutes/Playlist/Video.js");
//  Define All Route (Prakash Solanki)
app.use('/playlist_video',videoRoute);


//  End Import Route Files From Route Dir

mongoose.connect("mongodb://localhost/itr", { useNewUrlParser: true, useUnifiedTopology: true });
...
{
  try {
    if (req.path == "/login" || req.path == "/register" || req.path == "/" || req.path == "/addVideo" || req.path=="/playlist_video/add")
    {
      next();
    } else {
