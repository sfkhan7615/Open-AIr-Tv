...
const videoIndexx = (index) =>{
      ...
      .then((res) => {
        ...
        for (let index = 0; index < res.data.length; index++) {
          if (res.data[index].duration == 0) {
            ...
            axios
              .get(`https://www.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&id=` +VideoIdis +`&key=AIzaSyDOlMF3B52Ssd0wFzEZO8vaxgCoSCY-0v0`)
              .then((videores) => {
  ...
  // Get Location
  const [CurrentLocationData, SetCurrentLocationData] = useState((new Date()).toString().split('(')[1].split(" ")[0]);
    ...
    if (allPlaylistData.length != 0) {
      ...
      allPlaylistData.forEach((element, index) => {
        element.videos.forEach((LocalPlaylistData, index) => {
          ...
          {
            if (JSON.parse(LocalPlaylistData.location).country == CurrentLocationData
            ) {
    ...
    <>
      <main onClick={closeMenu}>
        <Grid container columns={{ xs: 4, sm: 8, md: 12 }}>
          <Grid sx={{ display: { xs: "none", sm: "block" } }} className="sideVideos Categories mt-2 borderRadius" item xs={6} sm={2} md={2}>
            <Item className="sideVideosHeading">Categories</Item>
            <div className="hideScrollBar" style={{ height: "31rem", overflow: "auto" }}>
              {Sports.map((val, index) => (
                <Grid onClick={() => setPlayUrl(val)} className="videoItem" item xs={12} sm={12} md={12} key={index}>
                  <Item className="aboutVideo transparent"><img className="playBtn" src={playBtn} alt="" />{val.category}</Item>
                  <Item className="videoImg transparent p-0"><img src={val.thumbnail[0]} alt="" /></Item>
                  <Item className="videoDesc transparent">{val.category}</Item>
                </Grid>
              ))}
              {Comedy.slice(0, 1).map((val, index) => (
                <Grid onClick={() => setPlayUrl(val)} style={{ cursor: "pointer" }} className="videoItem" item xs={12} sm={12} md={12} key={index}>
                  <Item className="aboutVideo transparent"><img className="playBtn" src={playBtn} alt="" />{val.category}</Item>
          ...
          {/* main Display */}
          <Grid style={{margin: "2px 17px 0px 17px", maxWidth:"63.666%"}} item xs={6} sm={4} md={8} className="displayVideoBox my-2 borderRadius todayList">
            <Item className="displayVideo py-2">
            <i className="bi bi-cloud-upload"></i>
                <div className="PlaylistPlayer__player-wrapper height-118" style={{position:'relative',overflow:'hidden'}}>
                  <ReactPlayer url={PlayUrl.url ? PlayUrl.url : ""} className="PlaylistPlayer__player" width="100%" height="100%" playing controls />
                  {topPlaylistData.length > 0 ? (<button id="suggestButton" onClick={()=> showSuggetionVideos?(setShowSuggetionVideos(false)):(setShowSuggetionVideos(true))} style={{position:'absolute',right:'0',top:'0',width:'100px',zIndex:'1'}}>{showSuggetionVideos?('Hide Video'):('Show Video')}</button>) : ("")}
                  
                  {
                    showSuggetionVideos ? (
                      <div className="suggestVideo" style={{position:'absolute',right:'0',top:'-9px',width:'100px'}}>
                      {topPlaylistData.slice(0, 10).map((val, index) =>
                        (
                          val.videos.length != 0 ? (
                            <div onClick={() => setPlayUrl(val.videos[0].url)}>
                              <img src={val.videos[0].thumbnail} style={{maxWidth:'100%' , cursor:'pointer'}} />
                            </div>
                          ) : ("")
                        ))}
                      </div>  
                ...
                <div style={{background:"#004D73" , height:"12.6%"}}>
                  <div className="row">
                    ...
                    {allData ? (<img
                          id="proFileImage"
                          src={"http://190.92.153.226:5000/assets/images/reserved/"+allData[0].resimage}
                          className="img-fluid"
                          alt=""
                          style={{ height: "65px", marginLeft:'12px' , width:"97%" }} 
                        />) : ("")}
                    
                    </div>
                    <div className="col col-md-4 font-size-9" style={{ background: "rgba(0,0,0,0.5)", color: "#fff"  }}>                     
                        <div> <marquee>{PlayUrl.title}</marquee> <br />{PlayUrl.channel_name}<br />{PlayUrl.posted_date}</div>                     
                    </div>
                    
                    <div className="col col-md-2 p-0 d-flex">
                    <ul className="p-0" style={{margin:'auto'}}>
                      <li style={{display:'inline',margin:'0'}}>
                          <img style={{ cursor: "pointer" , width:'30%' }} src={require(`../asset/images/logo.png`)} className="img-fluid" alt="" />
                      </li>
                        
                      <li style={{display:'inline',margin:'-2px'}}>
                          <img style={{ cursor: "pointer" , width:'30%' }} onClick={() => {setPlayUrl(allVideolistData[videoIndex + 1]) ; videoIndexx(videoIndex);}} src={require(`../asset/images/Button_play2.png`)} className="img-fluid" alt="" />
                      </li>

                      <li style={{display:'inline',margin:'0'}}>
                          <img style={{ cursor: "pointer" , width:'30%' }} src={require(`../asset/images/ButtonNext.png`)} className="img-fluid" alt="" />
                      </li>                                                                                                                 
                    </ul>
                  </div>
                    </div>
          ...
          {/* show on mobile */}
           <Grid sx={{ display: { xs: "block", sm: "none" } }} className="sideVideos Categories mt-2" item xs={6} sm={2} md={2}>
            <Item className="sideVideosHeading">Categories</Item>
            <div className="hideScrollBar" style={{ height: "31rem", overflow: "auto" }}>
              {Sports.map((val, index) => (
                <Grid onClick={() => setPlayUrl(val)} className="videoItem" item xs={12} sm={12} md={12} key={index}>
          ...
          {/* end show on mobile */}
          <Grid style={{ cursor: "pointer" }} className="sideVideos topPlaylists mt-2 borderRadius" item xs={6} sm={2} md={2}>
            <Item className="sideVideosHeading">Top playlists</Item>
            <div className="hideScrollBar" style={{ overflow: "auto", height: "31rem" }}>
              {topPlaylistData.slice(0, 10).map((val, index) =>
              (
                val.videos.length != 0 ? (
                  <Grid onClick={() => setPlayUrl(val.videos[0])} className="videoItem" item xs={12} sm={12} md={12} key={index}>
        ...
        <Grid container columns={{ xs: 4, sm: 8, md: 12 }} className="my-3 ">
          <Grid style={{ cursor: "pointer" }} className="sideVideos Categories borderRadius" item xs={6} sm={2} md={2}>
            <Item className="sideVideosHeading">Local</Item>
            <div className="hideScrollBar" style={{ overflow: "auto", height: "27rem" }}>
              {topPlaylistData.map((val, index) => (
                (val.videos.length != 0 ? <Grid onClick={() => setPlayUrl(val.videos[0])} className="videoItem" item xs={12} sm={12} md={12} key={index}>
                <Item className="aboutVideo transparent"><img className="playBtn" src={playBtn} alt="" />{val.title}</Item>
                <Item className="videoImg transparent p-0"><img src={val.videos[0] ? (val.videos[0].thumbnail) : ("") } alt="" /></Item>
            ...
            <Grid style={{ cursor: "pointer" }} className="videoItem" item xs={12} sm={12} md={12}>
              ...
              {allData ? 
              (<img
                src={"http://190.92.153.226:5000/assets/images/reserved/"+allData[1].resimage}
            ...
            <Grid style={{ cursor: "pointer" }} className="videoItem" item xs={12} sm={12} md={12}>
              ...
              (<img
                ...
                src={"http://190.92.153.226:5000/assets/images/reserved/"+allData[2].resimage}
                className="img-fluid resImageHome"
                alt=""
          ...
          <Grid style={{maxWidth:"63.666%" , margin:"2px 17px 0px 17px"}} item container xs={12} sm={12} md={8} className="sideVideos todayList borderRadius">
            ...
            <Grid style={{ cursor: "pointer" }} item container xs={12} sm={12} md={12} className="moreVideos">
              {
                allVideolistData.slice(0,24).map((videos, index) => (
                  videos.thumbnail !== ""  ? (
                  <Grid onClick={() => setPlayUrl(videos)} className="videoItem" item xs={12} sm={12} md={3} key={index}>
                  <Item className="aboutVideo transparent">
                    {index == 0 ? (
                      <><img className="playBtn" src={playBtn} alt="" />{videos.title.substring(0, 10)}</>
                    ) : (
                      <img className="playBtn" src={playBtn} alt="" style={{ visibility: "hidden" }} />
                    )}
                  </Item>
          ...
          <Grid style={{ cursor: "pointer" }} className="sideVideos topPlaylists borderRadius" item xs={6} sm={2} md={2}>
            ...
            <Grid style={{ cursor: "pointer", marginTop: '2px' }} className="videoItem" item xs={12} sm={12} md={12}>
              {allData ? 
              (<img
                
                src={"http://190.92.153.226:5000/assets/images/reserved/"+allData[3].resimage}
                className="img-fluid resImageHome"
                alt=""
            ...
            {allData ? 
              (<img
                ...
                src={"http://190.92.153.226:5000/assets/images/reserved/"+allData[4].resimage}
                className="img-fluid resImageHome"
                alt=""
