...
function Profile() {
  ...
  return (
    <>
      <Grid onClick={closeMenu} container className=" borderRadius todayList">
        <Grid item xs={12} className="transparent ">
          ...
          >
            {userPlaylist.map((val, index) => (
              <>
                <Grid
                  onClick={playlistWithId}
                  style={{ cursor: "pointer" }}
                  container
                  className="profileVideoes"
                  xs={12}
                  md={7.1}
                >
